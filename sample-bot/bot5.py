...
exit_profit_percent = 3
exit_loss_percent = 10
...
while True:
    try:
        ...
        value_yesterday = pub.get_candlestick('btc_jpy', '1hour', yesterday_date)
        ohlcv_yesterday = value_yesterday.get('candlestick', [{}])[0].get('ohlcv', [])
        ohlcv_combined += ohlcv_yesterday

        value_today = pub.get_candlestick('btc_jpy', '1hour', current_date)
